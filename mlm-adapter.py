...
from utils_glue import convert_examples_to_features_entity_mlm, output_modes, processors
...
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features_entity_mlm(
            examples,
            args.max_seq_length,
            tokenizer,
...
def main():
    ...
    parser.add_argument(
        "--adapter_skip_layers", default=0, type=int, help="The skip_layers of adapter according to bert layers"
    )
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model_to_save = (
            model.module if hasattr(adapter_model, "module") else adapter_model
        )  # Take care of distributed/parallel training
