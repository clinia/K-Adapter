...
from utils_glue import (
    convert_examples_to_features_trex,
    convert_examples_to_features_regression,
    output_modes,
    processors,
)
...
class AdapterModel(nn.Module):
    ...
    def forward(
        ...
        emb_a = hidden_states_last

        emb_a[token_type_ids == 0] = 0
        ...
        loss = self.loss_fn(emb_a.sum(1), labels).sum()
...
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features_regression(
            examples,
            args.max_seq_length,
            tokenizer,
            output_mode,
...
class KAdapterArgs(object):
    def __init__(self) -> None:
        ...
        self.data_dir = "./data/graph_data/regression_data"
        ...
        self.num_train_epochs = 600
        self.max_seq_lengt = 64
        self.gradient_accumulation_steps = 1
        self.learning_rate = 5e-4
        self.warmup_steps = 35
        self.save_steps = 100
        self.eval_steps = 10
